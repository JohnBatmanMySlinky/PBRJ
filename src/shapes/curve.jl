...
function recursive_intersect(
    ...
    ray_length = pbrt_length(ray.direction)
    si = nothing
    if depth > 0
        ...
        for seg in 0:1
            ...
            cps = SVector(sp_split[3*seg+1], sp_split[3*seg+2], sp_split[3*seg+3], sp_split[3*seg+4])
        ...
        segment_dir = Pnt2(cp[3+1].x, cp[3+1].y) - Pnt2(cp[0+1].x, cp[0+1].y)
        denom = length_squared(segment_dir)
        if denom == 0.0
            return false, nothing, nothing
        end
        w = dot(-Vec2(cp[0+1].x, cp[0+1].y), segmentDir) / denom
