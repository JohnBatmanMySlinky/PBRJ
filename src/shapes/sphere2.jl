...
# Implicit surface helper functions
function f(s::Sphere, p::Pnt3)::Float64
    center = Pnt3(0,0,0) # all the computations in intersect are in object space then get converted to world space
    return distance_squared(center, p) - s.radius^2
end

function normal(s::Sphere, p::Pnt3)::Nml3
    e = .00001
    return RayTracing.normalize(
        Vec3(1, -1, -1) * f(s, p + Vec3(e, -e, -e)) +
        Vec3(-1, -1, 1) * f(s, p + Vec3(-e, -e, e)) +
        Vec3(-1, 1, -1) * f(s, p + Vec3(-e, e, -e)) +
        Vec3(1, 1, 1) * f(s, p + Vec3(e, e, e))
    )
...
function intersect(s::Sphere, r::AbstractRay)::Tuple{Bool, Float64, SurfaceInteraction}
    ...
    u = 0.5
    v = 0.5
    dndu = Nml3(0.0, 1.0, 0.0)
    dndv = Nml3(1.0, 0.0, 0.0)
