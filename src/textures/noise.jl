...
struct NoiseTexture{T <: Union{Float64, Spectrum}} <: AbstractTexture{T}
    dummy::T
    scale::Float64
    omega::Float64
    maxOctaves::Int
...
# Main texture evaluation function
function (nt::NoiseTexture{Float64})(si::SurfaceInteraction)::Float64
    # Scale the surface position
    p = si.core.p * nt.scale
...
# For Spectrum types, you might want to return the same noise value across all channels
# or compute different noise for each channel
function (nt::NoiseTexture{Spectrum})(si::SurfaceInteraction)::Spectrum
    p = si.core.p * nt.scale
    dpdx = si.dpdx * nt.scale
    dpdy = si.dpdy * nt.scale
    ...
    return spectrum_from_float(noise_val)
end
