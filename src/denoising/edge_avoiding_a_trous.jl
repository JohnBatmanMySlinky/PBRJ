...
function denoise_kernel(
    ...
    for x in 1:X
        for y in 1:Y
            ...
            d_val = RayTracing.Pnt3(position_buffer[x, y, :])
            n_val = RayTracing.Pnt3(normal_buffer[x, y, :])

            # convolution loop
            L = RayTracing.Pnt3(0.0)
            cum_w = 0.0
            for dy in -2:2
                for dx in -2:2
                    ...
                    d_temp = RayTracing.Pnt3(position_buffer[x_temp, y_temp, :])
                    t = d_val - d_temp
                    dist2 = dot(t,t)
                    ...
                    L += c_temp * weight * kernel[kernel_index]
