...
######## Misc ##########################
########################################
function get_sample_bounds(f::PassFilm)::Bounds2
    return Bounds2(
...
function get_pixel(f::PassFilm, p::Pnt2)::Pixel
...
struct PassFilmTile
    ...
    filter_table::Matrix{Float64}
    filter_table_width::Int64
    pixels::Matrix{FilmTilePassPixel}

    function FilmTile(f::PassFilm, sample_bounds::Bounds2)
        ...
        pixels = [FilmTilePassPixel() for _ in 1:tile_res.y, __ in 1:tile_res.x]
...
function get_pixel(t::PassFilmTile, p::Pnt2)::FilmTilePassPixel
    pp = Int64.(p .- t.pixel_bounds.pMin .+ 1)
    return t.pixels[pp.y, pp.x]
end

function add_sample!(t::PassFilmTile, point::Pnt2, spectrum::S, sample_weight::Float64 = 1.0) where S <: Spectrum
...
function merge_film_tile!(f::PassFilm, ft::PassFilmTile)
...
function save(film::PassFilm, render_pass_flag::UInt8, splat_scale::Float64 = 1.0)::Array{Float64}
