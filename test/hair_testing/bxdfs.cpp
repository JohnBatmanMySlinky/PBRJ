...
HairBxDF::HairBxDF(Float h, Float eta, const SampledSpectrum &sigma_a, Float beta_m,
    ...
    std::cout << "sin2kAlpha: " << sin2kAlpha[0] << " " << sin2kAlpha[1] << " " << sin2kAlpha[2] << std::endl;
    std::cout << "cos2kAlpha: " << cos2kAlpha[0] << " " << cos2kAlpha[1] << " " << cos2kAlpha[2] << std::endl;
}
...
SampledSpectrum HairBxDF::f(Vector3f wo, Vector3f wi, TransportMode mode) const {
    ...
    for (int p = 0; p < pMax; ++p) {
        ...
        Float mp, np;
        mp = Mp(cosTheta_i, cosThetap_o, sinTheta_i, sinThetap_o, v[p]);
        std::cout << "mp: " << mp << std::endl;
        np = Np(phi, p, s, gamma_o, gamma_t);
        std::cout << "\tphi: " << phi << " p " << p << " s " << s << " gammao " << gamma_o << " gamma_t " << gamma_t << std::endl;
        std::cout << "np: " << np << std::endl;
        std::cout << "ap: " << ap[p] << std::endl;
        fsum += mp * ap[p] * np;
        std::cout << "fsum: " << fsum << std::endl;
    }
    // Compute contribution of remaining terms after _pMax_
    Float mp;
    mp = Mp(cosTheta_i, cosTheta_o, sinTheta_i, sinTheta_o, v[pMax]);
    std::cout << "mp: " << mp << std::endl;
    fsum += mp * ap[pMax] / (2 * Pi);
    std::cout << "fsum: " << fsum << std::endl;
